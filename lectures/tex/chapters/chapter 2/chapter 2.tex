...
\(f(a)\) và \(f(b)\) trái dấu.
\emph{\hyperref[thm:intermediate_value_theorem]{Định lý giá trị trung gian}},
hay còn gọi là \emph{định lý Bolzano}, nói rằng tồn tại một số \(p\) trong
\(\interval[open]{a}{b}\) với \(f(p) = 0\).
...
\begin{enumerate}
    \item Nếu \(g(a) = a\) hoặc \(g(b) = b\), \(g\) có điểm bất động \(a\) hoặc
        ...
        Định lý giá trị trung gian~\ref{thm:intermediate_value_theorem} khẳng
        định rằng tồn tại \(p \in \interval[open]{a}{b}\) sao cho \(h(p) = 0\).
        Điểm \(p\) này là điểm bất động của \(g\) vì
        \[0 = h(p) = g(p) - p \iff g(p) = p\]

    \item Giả sử \(g\) có hai điểm bất động \(p\), \(q\) trên
        \(\interval{a}{b}\). Không mất tính tổng quát, giả sử \(p < q\). Theo
        định lí giá trị trung bình~\ref{thm:mean_value_theorem}, tồn tại \(\xi
        \in \interval[open]{p}{q}\) sao cho:
