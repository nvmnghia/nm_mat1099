...
hàm số \(y = x\), vì vậy \(g\) có 2 điểm bất động là \(-1\) và \(2\). Điều này
...
\phantom\\
\begin{itemize}
    \item Nếu \(g \in \interval{a}{b}\), và \(g(x) \in \interval{a}{b} \forall x
        \in \interval{a}{b}\), khi đó \(g\) có ít nhất một điểm bất động trên
        \(\interval{a}{b}\).
    \item Hơn nữa, nếu \(g'(x)\) tồn tại trên \(\interval[open]{a}{b}\) và tồn
        tại hằng số dương \(k < 1\) thỏa \(\abs{g'(x)} \leq k \forall x \in
        \interval{a}{b}\), khi đó, tồn tại đúng một điểm bất động trên
        \(\interval{a}{b}\).
\end{itemize}
\end{theorem}
