...
class FalsePositionIterData(NamedTuple):
    """
...
def false_position(P_0: float, P_1: float, ERROR_BOUND: float, MAX_ITER: int, return_all=False, **_) -> List[FalsePositionIterData]:
    """
    ...
    try:
        ...
        f_p_0 = f(p_0)
        if return_all:
            T.append(FalsePositionIterData(n=0, p=p_0, f_p=f_p_0))

        p_1 = P_1
        f_p_1 = f(p_1)
        if return_all:
            T.append(FalsePositionIterData(n=1, p=p_1, f_p=f_p_1))

        if same_sign(f_p_0, f_p_1):
            logging.warning(f'f(P_0) = {f_p_0} and f(P_1) = {f_p_1} same sign, convergence not guaranteed')
        else:
            logging.info(f'f(P_0) = {f_p_0} and f(P_1) = {f_p_1} opposite signs, convergence guaranteed')
    ...
    for i in range(2, MAX_ITER + 1):
        ...
        if return_all:
            T.append(FalsePositionIterData(n=i, p=p, f_p=f_p))

        if abs(delta) < ERROR_BOUND:
            found = True
            break

        if not same_sign(f_p, f_p_1):
            p_0 = p_1
            f_p_0 = f_p_1
        p_1 = p
        f_p_1 = f_p
