...
\begin{exercise}
    ...
    Suppose \(L = \qty{10}{ft}\), \(r = \qty{1}{ft}\), and \(V =
    \qty{12.4}{ft^3}\). Find the depth of water in the trough to within
    \(\qty{0.01}{ft}\).
\end{exercise}
...
\begin{solution}
    ...
    \(\qty{0.01}{ft}\). The number of iteration \(n\) needed to approximate
...
\begin{exercise}
    ...
    Suppose the particle has moved \qty{1.7}{ft} in \qty{1}{s}. Find, to within
    \(10^{5}\), the rate \(\omega\) at which \(\theta\) changes. Assume that \(g
    = \qty{32.17}{ft \per \second \squared}\).
\end{exercise}

\begin{solution}
    As \(\omega < 0\), the plane rotates clockwise. After \qty{1}{\s}, the
    particle still sticks to the plane, so:

    \[\theta(1) < \frac{\pi}{2} \iff - \frac{\pi}{2} < \omega < 0\]

    After \qty{1}{\s}, the particle has moved \qty{1.7}{ft}, so that:
    ...
    \begin{longtable}{r S[table-format=-1.9] S[table-format=-1.9] S[table-format=-1.9] S[table-format=-1.9]}
        ...
        \midrule
        \endhead
            1  &  -1.57079633   &   1            &  -0.285398163  &   0.027657569   \\
